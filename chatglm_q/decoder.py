...
def top_p_sampling(logits: torch.Tensor, p=0.8):
    probs = torch.softmax(logits.float(), dim=-1)
    probs, indices = torch.sort(probs, dim=-1, descending=True)
    cumsum = torch.cumsum(probs, dim=-1)
    probs[(cumsum - probs) > p] = 0.0
...
class ChatGLMDecoder():
    def __init__(self, model: ChatGLMModel, tokenizer: ChatGLMTokenizer, top_p = 0.8, device = None):
        self.model = model
    ...
    def generate(self, prefix_text: str, eos_token_id: int, max_length = 200):
        model, tokenizer, top_p = self.model, self.tokenizer, self.top_p
        ...
        while len(input_ids) < max_length:
            ...
            next_token = top_p_sampling(logits[:, -1], top_p).to("cpu")
